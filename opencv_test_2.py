...
imgs = []
loaded,imgs = cv2.imreadmulti("CHOLD.tif", 9, 1, imgs, cv2.IMREAD_COLOR)
for x in imgs:
    # 保存原始圖片
    origin_img = x
    # 先將圖片縮小進行處理
    ...
    dst_mask = (np.ones(img.shape, dtype=np.uint8)*255) - dst #1矩陣*255 減去蒙版等於取反
    show_img(dst_mask)

    # resize 是要填(x寬,y高)，img.shape提供的是(高,寬,通道數)，img.shape在搞==
    # 此處是將蒙版縮放回原始圖像的大小，並與原始圖像合併
    dst_mask = cv2.resize(dst_mask,(origin_img.shape[1],origin_img.shape[0]))
    img_add = cv2.add(origin_img, dst_mask)
    show_img(img_add)
